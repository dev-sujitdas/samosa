export const games = [
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/transforming-mars.webp",
    name: "Terraforming Mars",
    type: "Base",
    difficulty: "Expert",
    duration: ">90",
    tags: ["Deck-building"],
    players: [1, 2, 3, 4, 5],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/catan.webp",
    name: "Catan",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["resource management"],
    players: [3, 4, 5, 6],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/catan new.webp",
    name: "Catan New Energies",
    type: "Version",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["resource management"],
    players: [3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/dune imperium.webp",
    name: "Dune Imperium",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["Deck-building", "worker placement"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/powergrid.webp",
    name: "Power grid",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["resource management"],
    players: [2, 3, 4, 5, 6],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/Splendour.webp",
    name: "Splendour",
    type: "Base",
    difficulty: "Medium",
    duration: "30-60",
    tags: ["resource management"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/Splendour expansions.webp",
    name: "Splendour expansions",
    type: "Expansion",
    difficulty: "Easy",
    duration: "<30",
    tags: ["resource management"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/root.webp",
    name: "Root",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["assymetric"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/turing.webp",
    name: "Turing Machine",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["Analytical"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/gallilio.webp",
    name: "Galileo Galilei",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["worker placement"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/marie.webp",
    name: "Marie Curie",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["resource management"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/wonders duel.webp",
    name: "7 wonders Duel",
    type: "Version",
    difficulty: "Expert",
    duration: "<30",
    tags: ["Analytical", "resource management"],
    players: [2],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/deep sea.webp",
    name: "Deep Sea Adventure",
    type: "Base",
    difficulty: "Easy",
    duration: "30-60",
    tags: ["dice", "resource management"],
    players: [2, 3, 4, 5, 6],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/bandido.webp",
    name: "Bandido",
    type: "Version",
    difficulty: "Easy",
    duration: "<30",
    tags: ["Analytical"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/sea salt.webp",
    name: "sea salt & paper",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["card game"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/extra salt.webp",
    name: "extra salt",
    type: "Expansion",
    difficulty: "Easy",
    duration: "<30",
    tags: ["card game"],
    players: [2, 3, 4, 5, 6],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/lama.webp",
    name: "Lama",
    type: "Base",
    difficulty: "Expert",
    duration: "30-60",
    tags: ["card game"],
    players: [2, 3, 4, 5],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/up to.webp",
    name: "Up too high",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["card game"],
    players: [3, 4, 5, 6, 7],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/startups.webp",
    name: "Startups",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["Analytical", "card game"],
    players: [2, 3, 4, 5],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/scout.webp",
    name: "Scout",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["card game"],
    players: [3, 4, 5, 6, 7, 8],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/fuji flush.webp",
    name: "Fuji Flush",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["card game"],
    players: [1, 2, 3, 4, 5, 6, 7, 8, 9],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/uno flip.webp",
    name: "Uno Flip",
    type: "Version",
    difficulty: "Easy",
    duration: "<30",
    tags: ["card game"],
    players: [1, 2, 3, 4, 5, 6, 7, 8, 9],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/uno.webp",
    name: "Uno",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["matching"],
    players: [2, 3, 4, 5, 6],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/double.webp",
    name: "Double",
    type: "Version",
    difficulty: "Easy",
    duration: "<30",
    tags: ["Analytical"],
    players: [2, 3, 4, 5],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/qwixx.webp",
    name: "Qwixx",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["party"],
    players: [1, 5, 6, 7, 8, 9],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/fake artist.webp",
    name: "Fake Artist goes to New York",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["party"],
    players: [4, 5, 6, 7, 8],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/insider.webp",
    name: "Insider",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["party"],
    players: [1, 2, 3, 4, 5, 6, 7, 8, 9],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/hey yo.webp",
    name: "Hey Yo",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: [],
    players: [3, 4, 5, 6, 7],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/rights.webp",
    name: "Rights",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["memory", "party"],
    players: [2, 3, 4, 5, 6],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/beverbende.webp",
    name: "Beverbende",
    type: "Version",
    difficulty: "Easy",
    duration: "<30",
    tags: ["matching", "party"],
    players: [2, 3, 4, 5],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/afzaker.webp",
    name: "Afzaker",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["resource management"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/catan dice.webp",
    name: "Catan Dice play",
    type: "Base",
    difficulty: "Medium",
    duration: "<30",
    tags: [],
    players: [2, 3, 4, 5],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/dirt.webp",
    name: "Dirt and Glory",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["card game"],
    players: [3, 4, 5, 6],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/pot.webp",
    name: "Pot de Vin",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["Analytical"],
    players: [2, 3],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/bus.webp",
    name: "Bus",
    type: "Base",
    difficulty: "Expert",
    duration: "<30",
    tags: ["Analytical"],
    players: [2],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/hive.webp",
    name: "Hive",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["Analytical"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/rummikub.webp",
    name: "Rummikub",
    type: "Version",
    difficulty: "Medium",
    duration: "<30",
    tags: ["Analytical", "dice"],
    players: [2],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/cleocatra.webp",
    name: "Cleocatra",
    type: "Base",
    difficulty: "Expert",
    duration: "30-60",
    tags: ["resource management"],
    players: [2, 3, 4, 5],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/sky team.webp",
    name: "Sky Team",
    type: "Base",
    difficulty: "Expert",
    duration: "30-60",
    tags: ["Analytical", "resource management"],
    players: [2, 3, 4, 5],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/century spice.webp",
    name: "Century Spice",
    type: "Base",
    difficulty: "Expert",
    duration: "30-60",
    tags: ["resource management", "worker placement"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/pandemic Iberia.webp",
    name: "Pandemic Iberia",
    type: "Version",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["Analytical", "resource management", "worker placement"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/darwin journey.webp",
    name: "Darwin’s Journey",
    type: "Base",
    difficulty: "Expert",
    duration: "30-60",
    tags: ["resource management", "worker placement"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/alchemists.webp",
    name: "Alchemists",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["Analytical", "resource management", "worker placement"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/darwin fireland.webp",
    name: "Darwin’s Journey Expansion: Fireland",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["Analytical", "party"],
    players: [2],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/king golem.webp",
    name: "Alchemist Expansion: King’s Golem",
    type: "Version",
    difficulty: "Easy",
    duration: "30-60",
    tags: ["party"],
    players: [4, 5, 6, 7],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/lost cities.webp",
    name: "Lost Cities",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["Analytical", "resource management", "worker placement"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/bang.webp",
    name: "Bang",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["assymetric", "resource management"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/epic tactics.webp",
    name: "Epic Tactics",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["resource management"],
    players: [2, 3, 4, 5, 6],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/root.webp",
    name: "Root: The Clockwork Expansion",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["resource management"],
    players: [2, 3, 4, 5, 6],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/power grid.webp",
    name: "Power grid Expansion: China/Korea",
    type: "Base",
    difficulty: "Expert",
    duration: ">90",
    tags: ["resource management"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/powergrid.webp",
    name: "Power grid Expansion: Middle East/South Africa",
    type: "Version",
    difficulty: "Easy",
    duration: "<30",
    tags: ["puzzle"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/ark nova.webp",
    name: "Ark Nova",
    type: "Base",
    difficulty: "Medium",
    duration: "<30",
    tags: ["puzzle"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/kingdomino.webp",
    name: "Kingdomino",
    type: "Base",
    difficulty: "Medium",
    duration: "30-60",
    tags: ["card game", "memory", "resource management"],
    players: [2],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/cascadia.webp",
    name: "Cascadia",
    type: "Base",
    difficulty: "Medium",
    duration: "30-60",
    tags: ["Analytical", "puzzle"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/bread.webp",
    name: "Bread and bier",
    type: "Base",
    difficulty: "Expert",
    duration: ">90",
    tags: ["Deck-building", "worker placement"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/azul mini.webp",
    name: "Azul mini",
    type: "Base",
    difficulty: "Expert",
    duration: "",
    tags: ["Deck-building", "worker placement"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/dune immortality.webp",
    name: "Dune Imperium: Immortality",
    type: "Expansion",
    difficulty: "Medium",
    duration: "<30",
    tags: ["card game", "trick taking"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/dune rise of ix.webp",
    name: "Dune Imperium: Rise of Ix",
    type: "Expansion",
    difficulty: "Easy",
    duration: "<30",
    tags: ["Analytical"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/lord ring.webp",
    name: "Lord of the rings trick taking",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["resource management", "worker placement"],
    players: [2, 3, 4, 5],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/rumble nation.webp",
    name: "Rumble nation ",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["resource management", "worker placement"],
    players: [2, 3, 4, 5],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/concordia.webp",
    name: "Concordia",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["resource management"],
    players: [2, 3, 4, 5, 6],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/pixies.webp",
    name: "Pixies",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["resource management"],
    players: [2, 3, 4, 5, 6],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/yokai pagoda.webp",
    name: "Yokai Pagoda",
    type: "Version",
    difficulty: "Easy",
    duration: "<30",
    tags: ["puzzle"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/public enemy.webp",
    name: "public enemy number one",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["abstract"],
    players: [2, 3, 4, 5, 6],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/japan.webp",
    name: "Let’s go to Japan",
    type: "Base",
    difficulty: "Expert",
    duration: ">90",
    tags: [],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/6 language.webp",
    name: "I speak 6 languages",
    type: "Base",
    difficulty: "Expert",
    duration: ">90",
    tags: ["Analytical", "resource management", "worker placement"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/pandemic legacy.webp",
    name: "Pandemic legacy season 0",
    type: "Base",
    difficulty: "Easy",
    duration: "30-60",
    tags: ["abstract", "strategy"],
    players: [2, 3, 4, 5, 6, 7, 8],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/tycoon.webp",
    name: "Tycoon: India 1981",
    type: "Base",
    difficulty: "Medium",
    duration: "60-90",
    tags: ["abstract", "strategy"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/tacta.webp",
    name: "Tacta",
    type: "Expansion",
    difficulty: "Medium",
    duration: "<30",
    tags: ["memory", "strategy"],
    players: [2],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/lumina spell.webp",
    name: "Lumina Spell",
    type: "Expansion",
    difficulty: "Medium",
    duration: "30-60",
    tags: ["Deck-building"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/tag team.webp",
    name: "Tag Team",
    type: "Base",
    difficulty: "Medium",
    duration: "30-60",
    tags: ["abstract", "puzzle"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/eternity.webp",
    name: "The vale of eternity",
    type: "Base",
    difficulty: "Medium",
    duration: "60-90",
    tags: ["Deck-building"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/knitting circle.webp",
    name: "Knitting circle",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["resource management", "worker placement"],
    players: [1, 2, 3, 4, 5],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/terraforming mars.webp",
    name: "Terraforming Mars: Ares Expedition",
    type: "Version",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["Deck-building"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/agricola.webp",
    name: "Agricola",
    type: "Base",
    difficulty: "Expert",
    duration: ">90",
    tags: ["Deck-building"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/automa.webp",
    name: "Terraforming Mars: Automa",
    type: "Base",
    difficulty: "Expert",
    duration: "60-90",
    tags: ["Deck-building"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/turmoil.webp",
    name: "Terraforming Mars: turmoil",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy", "trick taking"],
    players: [3, 4, 5],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/prelude.webp",
    name: "Terraforming Mars: Prelude",
    type: "Version",
    difficulty: "Medium",
    duration: "",
    tags: ["trick taking"],
    players: [3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/fearless.webp",
    name: "Fearless",
    type: "Base",
    difficulty: "Expert",
    duration: "30-60",
    tags: ["Deck-building"],
    players: [2],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/bid coin.webp",
    name: "Bid coin",
    type: "Base",
    difficulty: "Medium",
    duration: "<30",
    tags: ["Deck-building"],
    players: [1, 2, 3, 4, 5],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/star realms.webp",
    name: "Star realms: Frontiers",
    type: "Expansion",
    difficulty: "Medium",
    duration: "R M",
    tags: [],
    players: [2],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/108 outlaws.webp",
    name: "108 outlaws",
    type: "Expansion",
    difficulty: "Easy",
    duration: "<30",
    tags: ["trick taking"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/splendor duel.webp",
    name: "Splendor: Duel",
    type: "Expansion",
    difficulty: "Easy",
    duration: "<30",
    tags: ["card game", "memory", "strategy"],
    players: [2],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/for northwood.webp",
    name: "For northwood",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["card game", "memory", "strategy"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/halli galli.webp",
    name: "Halli Galli",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/jekyll.webp",
    name: "Jekyll & Hyde",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/kingdom legacy.webp",
    name: "Kingdom Legacy: Feudal Kingdoms",
    type: "Expansion",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/distant lands.webp",
    name: "Kingdom Legacy: Distant Lands",
    type: "Expansion",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/gloomhaven.webp",
    name: "Gloomhaven : Buttons and Bugs",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/feudal foundation.webp",
    name: "Kingdom Legacy: Feudal Kingdoms Foundations",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: [],
    players: [1],
  },
  {
    default: "/game-icons/bluffit.webp",
    icon: "/game-icons/bluff it.webp",
    name: "Bluff It",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/eldorado.webp",
    name: "The Quest for El dorado",
    type: "Version",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/robotrick.webp",
    name: "Robotrick",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/ridding woods.webp",
    name: "Kingdom Legacy: Ridding the Woods",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/ambition.webp",
    name: "Kingdom Legacy: Ambitions",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/adventure.webp",
    name: "Kingdom Legacy: Adventures",
    type: "Expansion",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/merchant.webp",
    name: "Kingdom Legacy: Merchants",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/fishing lessons.webp",
    name: "Fishing lessons",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: [],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/fishing expansion.webp",
    name: "Fishing lessons expansion",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/unsurmountable.webp",
    name: "unsurmountable",
    type: "Expansion",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/unsurmountable expansion.webp",
    name: "unsurmountable expansion",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/hyperstar run.webp",
    name: "Hyperstar Run",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/hyperstar equipment.webp",
    name: "Hyperstar Run Equipment Drop",
    type: "Expansion",
    difficulty: "Easy",
    duration: "<30",
    tags: [],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/hyperstar final.webp",
    name: "Hyperstar Run Final Bosses",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/royal.webp",
    name: "The Royal Limited",
    type: "Expansion",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/ugly.webp",
    name: "ugly Gryphon Inn",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: [],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/ugly expansion.webp",
    name: "ugly Gryphon Inn expansion",
    type: "Expansion",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/ancient realm.webp",
    name: "Ancient realm",
    type: "Base",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/sprawlopolis.webp",
    name: "Sprawlopolis",
    type: "Sprawlopolis",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/agropolis.webp",
    name: "Agropolis: Sprawlopolis",
    type: "Agropolis: Sprawlopolis",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1, 2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/mysticana.webp",
    name: "Mysticana",
    type: "Mysticana",
    difficulty: "Easy",
    duration: "<30",
    tags: ["strategy"],
    players: [1, 2, 3, 4],
  },


  // new list added 02-03-2026

   {
    default: "/game-icons/default.webp",
    icon: "/game-icons/domino.webp",
    name: "Domino",
    type: "Base",
    difficulty: "Easy",
    duration: ">30",
    tags: ["tile game", "classic", "family"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/cluedo.webp",
    name: "Cluedo",
    type: "Base",
    difficulty: "Medium",
    duration: ">30",
    tags: ["mystery", "deduction", "strategy"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/monopoly.webp",
    name: "Monopoly",
    type: "Base",
    difficulty: "Easy",
    duration: ">30",
    tags: ["economy", "resource management", "family"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/ludo checkers.webp",
    name: "Ludo Checkers",
    type: "Base",
    difficulty: "Easy",
    duration: ">30",
    tags: ["classic", "family", "casual"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/point salad.webp",
    name: "Point Salad",
    type: "Base",
    difficulty: "Easy",
    duration: ">30",
    tags: ["card drafting", "casual", "strategy"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/backgammon.webp",
    name: "Backgammon",
    type: "Base",
    difficulty: "Medium",
    duration: ">30",
    tags: ["abstract", "strategy", "classic"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/texas hold’em poker.webp",
    name: "Texas Hold’em Poker",
    type: "Base",
    difficulty: "Medium",
    duration: ">30",
    tags: ["card game", "bluffing", "strategy"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/siberian manhunt.webp",
    name: "Siberian Manhunt",
    type: "Base",
    difficulty: "Expert",
    duration: ">30",
    tags: ["hidden movement", "strategy", "asymmetric"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/connect 4.webp",
    name: "Connect 4",
    type: "Base",
    difficulty: "Easy",
    duration: ">30",
    tags: ["abstract", "family", "classic"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/lene, meine, mij.webp",
    name: "Lene, Meine, Mij",
    type: "Base",
    difficulty: "Easy",
    duration: ">30",
    tags: ["party", "children", "family"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/junior scrabble.webp",
    name: "Junior Scrabble",
    type: "Base",
    difficulty: "Easy",
    duration: ">30",
    tags: ["word game", "educational", "family"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/bomb busters.webp",
    name: "Bomb Busters",
    type: "Base",
    difficulty: "Medium",
    duration: ">30",
    tags: ["party", "dexterity", "real-time"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/sagrada.webp",
    name: "Sagrada",
    type: "Base",
    difficulty: "Medium",
    duration: ">30",
    tags: ["dice drafting", "strategy", "abstract"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/bohnanza.webp",
    name: "Bohnanza",
    type: "Base",
    difficulty: "Medium",
    duration: ">30",
    tags: ["card trading", "negotiation", "strategy"],
    players: [2, 3, 4],
  },
  {
    default: "/game-icons/default.webp",
    icon: "/game-icons/take 5.webp",
    name: "Take 5",
    type: "Base",
    difficulty: "Easy",
    duration: ">30",
    tags: ["card game", "push your luck", "family"],
    players: [2, 3, 4],
  },
  {
  default: "/game-icons/default.webp",
  icon: "/game-icons/flip 7.webp",
  name: "Flip 7",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["card game", "party", "casual"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/the game.webp",
  name: "The Game",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["cooperative", "card game", "strategy"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/power hungry pets.webp",
  name: "Power Hungry Pets",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["card game", "family", "casual"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/7 wonders.webp",
  name: "7 Wonders",
  type: "Base",
  difficulty: "Expert",
  duration: ">30",
  tags: ["engine building", "card drafting", "strategy"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/the white castle.webp",
  name: "The White Castle",
  type: "Base",
  difficulty: "Expert",
  duration: ">30",
  tags: ["resource management", "strategy", "eurogame"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/dorfromantik.webp",
  name: "Dorfromantik",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["tile placement", "cooperative", "relaxing"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/exploding kittens.webp",
  name: "Exploding Kittens",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["party", "card game", "humor"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/exploding kittens nsfw.webp",
  name: "Exploding Kittens NSFW",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["party", "card game", "adult"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/koehandel.webp",
  name: "Koehandel",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["auction", "negotiation", "family"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/hengelen.webp",
  name: "Hengelen",
  type: "Base",
  difficulty: "Medium",
  duration: ">30",
  tags: ["dexterity", "family", "casual"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/wavelength.webp",
  name: "Wavelength",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["party", "team game", "communication"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/really loud librarians.webp",
  name: "Really Loud Librarians",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["party", "word game", "chaotic"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/trio.webp",
  name: "Trio",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["card game", "memory", "family"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/codenames.webp",
  name: "Codenames",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["word game", "team play", "deduction"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/cards against humanity.webp",
  name: "Cards Against Humanity",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["party", "adult", "humor"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/nuns on the run.webp",
  name: "Nuns on the Run",
  type: "Base",
  difficulty: "Medium",
  duration: ">30",
  tags: ["hidden movement", "strategy", "family"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/ticket to ride.webp",
  name: "Ticket to Ride",
  type: "Base",
  difficulty: "Medium",
  duration: ">30",
  tags: ["route building", "strategy", "family"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/ticket to ride amsterdam.webp",
  name: "Ticket to Ride Amsterdam",
  type: "Base",
  difficulty: "Medium",
  duration: ">30",
  tags: ["route building", "family", "strategy"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/ticket to ride netherlands.webp",
  name: "Ticket to Ride Netherlands",
  type: "Base",
  difficulty: "Medium",
  duration: ">30",
  tags: ["route building", "strategy", "advanced"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/captain flip.webp",
  name: "Captain Flip",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["card game", "push your luck", "family"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/jaipur.webp",
  name: "Jaipur",
  type: "Base",
  difficulty: "Medium",
  duration: ">30",
  tags: ["card game", "trading", "strategy"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/agent avenue.webp",
  name: "Agent Avenue",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["two player", "bluffing", "strategy"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/duel for cardia.webp",
  name: "Duel for Cardia",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["card game", "duel", "strategy"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/patchwork.webp",
  name: "Patchwork",
  type: "Base",
  difficulty: "Medium",
  duration: ">30",
  tags: ["tile placement", "strategy", "abstract"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/el grande.webp",
  name: "El Grande",
  type: "Base",
  difficulty: "Medium",
  duration: ">30",
  tags: ["area control", "strategy", "classic"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/toy battle.webp",
  name: "Toy Battle",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["family", "strategy", "casual"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/hanami koji.webp",
  name: "Hanami Koji",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["card game", "bluffing", "strategy"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/guillotine.webp",
  name: "Guillotine",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["card game", "take that", "strategy"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/dixit.webp",
  name: "Dixit",
  type: "Base",
  difficulty: "Easy",
  duration: ">30",
  tags: ["storytelling", "party", "creative"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/lord of the rings the two towers.webp",
  name: "Lord of the Rings: The Two Towers (Trick Taking)",
  type: "Base",
  difficulty: "Medium",
  duration: ">30",
  tags: ["trick taking", "card game", "thematic"],
  players: [2, 3, 4],
},
{
  default: "/game-icons/default.webp",
  icon: "/game-icons/cosmic encounter.webp",
  name: "Cosmic Encounter",
  type: "Base",
  difficulty: "Expert",
  duration: ">30",
  tags: ["asymmetric", "strategy", "negotiation"],
  players: [2, 3, 4],
},
];
